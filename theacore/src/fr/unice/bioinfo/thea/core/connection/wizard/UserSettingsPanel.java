...
/**
 * A swing GUI that allows users to enter a login and a password 
 * to establish a connection to a given data base server.
...
public class UserSettingsPanel extends JPanel implements PropertyChangeListener {
    ...
    private void init(JPanel self) {
        ...
        usernameTxtFld.setText(ConnectionSettings.getInstance().getUsername());
        constraints.gridx = 1;
        ...
        passwordFld.setText(ConnectionSettings.getInstance().getPassword());
        constraints.gridx = 1;
    ...
    public void initFromSettings(WizardDescriptor wd) {
        ...
        //        usernameTxtFld.setText(settings.getUsername());
        //        passwordFld.setText("");
    }
    ...
    public void propertyChange(PropertyChangeEvent evt) {
        if (evt.getSource() instanceof WizardDescriptor) {
            ConnectionSettings settings = ConnectionSettings.getInstance();
            settings.setUsername(usernameTxtFld.getText());
            settings.setPassword(new String(passwordFld.getPassword()));
        }
    }
